...
Menu cho phép chọn một solver, nhiều solver hoặc tất cả solver. Nếu chọn `cp_rolling`, menu chỉ hỏi rolling time limit mỗi ngày, số candidate mỗi ngày và số worker; các tùy chọn nâng cao vẫn nằm trong CLI.
...
- CP rolling defaults: `candidate_strategy="hybrid"`, `use_service_no_overlap=False`, `use_decision_strategy=True`, `solve_phase2=True`.
